...
object OnlineCoursePlatform:
  ...
  private case class OnlineCoursePlatformImpl() extends OnlineCoursePlatform {
    private type Student = String
    private var catalog: Sequence[Course] = Sequence()
    private var enrollments: Sequence[(Student, Course)] = Sequence()
    /**
     * Adds a new course to the platform's catalog.
     *
     * @param course The course to add.
    ...
    override def addCourse(course: Course): Unit =
      this.catalog = this.catalog.concat(Sequence(course))
    /**
     * Finds courses belonging to a specific category.
     *
     * @param category The category to search for.
    ...
    override def findCoursesByCategory(category: String): Sequence[Course] =
      this.catalog.filter(c => c.category == category)
    ...
    override def getCourse(courseId: String): Optional[Course] =
      this.catalog.find(c => c.courseId == courseId)
    ...
    override def removeCourse(course: Course): Unit =
      this.catalog = this.catalog.filter(c => c != course)
    ...
    override def isCourseAvailable(courseId: String): Boolean =
      !this.catalog.find(c => c.courseId == courseId).isEmpty
    ...
    override def enrollStudent(studentId: String, courseId: String): Unit = getCourse(courseId) match
      ...
      case Optional.Just(c) => this.enrollments = this.enrollments.concat(Sequence((studentId, c)))
    ...
    override def unenrollStudent(studentId: String, courseId: String): Unit =
      this.enrollments = this.enrollments.filter((stud, course) => (stud, course.courseId) != (studentId, courseId))
    ...
    override def getStudentEnrollments(studentId: String): Sequence[Course] =
      this.enrollments.filter((stud, _) => stud == studentId).map((_, course) => course)
    ...
    override def isStudentEnrolled(studentId: String, courseId: String): Boolean =
      this.enrollments.find((stud, course) => (stud, course.courseId) != (studentId, courseId)).isEmpty
}
/**
